...
def early_stopping(patience, loss_array):
    if(len(loss_array) > patience):
        ref_loss = loss_array[len(loss_array) - patience - 1]
        count = 0
        for i in range(len(loss_array) - patience, len(loss_array)):
            if(loss_array[i] >= ref_loss):
                count += 1
        if(count == patience):
            return 1
        else:
...
def validation_loop(dataloader, model, loss_fn, acc_array, patience, loss_array):
    ...
    with torch.no_grad():                               # Disabilita il calcolo dei gradienti che avverrebbe con la loss_fn (qui non serve farlo)
        for X, y in dataloader:                    # Itera sui batch del test set
            pred = model(X)
            y = y.squeeze(1)                             # Esegue la previsione
    ...
    if(early_stopping(patience, loss_array) == 1):
        return 1
...
def plot_results(rounds_array, array_iid, array_non_iid, client_nns, titolo, ordinata):
    plt.plot(rounds_array, array_iid, marker='o', label='IID')
    # x_max = rounds_array[np.argmax(array_iid)]
    # y_max = array_iid[np.argmax(array_iid)]
    # plt.scatter(x_max, y_max, color='red', edgecolor='black', s=80, zorder=5)
    # plt.text(x_max, y_max + 0.02, f"{y_max:.2f}", color='red', ha='center', fontsize=9)
    
    plt.plot(rounds_array, array_non_iid, marker='s', label='Non-IID')
    # x_max = rounds_array[np.argmax(array_iid)]
    # y_max = array_non_iid[np.argmax(array_iid)]
    # plt.scatter(x_max, y_max, color='red', edgecolor='black', s=80, zorder=5)
    # plt.text(x_max, y_max + 0.02, f"{y_max:.2f}", color='red', ha='center', fontsize=9)
    
    plt.xlabel("Round/Epoch")
    plt.ylabel(ordinata)
    plt.title(titolo + " - " + str(client_nns) + " clients")
    plt.legend()
    plt.grid(True)
