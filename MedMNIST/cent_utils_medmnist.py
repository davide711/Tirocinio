...
def train_loop(train_dataloader, model, loss_fn, optimizer):
    ...
    for _, (X, y) in enumerate(train_dataloader):  # Itera sui batch: X = immagini, y = etichette (così stampa i risultati ogni 100 batch)
        pred = model(X)
        y = y.squeeze(1)                         # Esegue la previsione del modello
...
def plot_results(rounds_array, result_array, client_nns, titolo, ordinata):
    plt.plot(rounds_array, result_array, marker='^', label='Centralizzato')
